...
- Any of the 95 PyHC Python packages (in `pyhc_packages/`) and community resources (in `pyhc_resources/`)
...
This will download all 95 PyHC package repositories (~several GB).
...
```
pyhc-chat-cli/
├── pyhc_packages/      # 95 PyHC packages as git submodules
...
95 PyHC packages including:

**Core Packages:**
- HAPI Client, Kamodo, PlasmaPy, pysat, pySPEDAS, SpacePy, SunPy

**And 88 specialized packages** for solar physics, magnetosphere science, ionosphere/thermosphere research, data access, visualization, and more.
